...
export default function WelcomeCarousel({
  onDismiss, totalUsers = 0, totalMatches = 0, isPro = false,
}: {
  onDismiss:     () => void;
  totalUsers?:   number;
  totalMatches?: number;
  ...
  // Pro users see 5 slides; free users see 5 image slides + 1 upgrade slide
  const TOTAL = isPro ? IMG_TOTAL : IMG_TOTAL + 1;
  ...
  const isProSlide = !isPro && step === IMG_TOTAL; // slide 6 — pro upgrade
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* ── Pro upgrade slide (slide 6, free users only) ── */}
        {!isPro && (
          <div style={{
            position: "absolute", inset: 0,
            opacity: isProSlide ? 1 : 0,
            transition: "opacity 0.4s ease",
            pointerEvents: isProSlide ? "all" : "none",
            display: "flex", alignItems: "center", justifyContent: "center",
            background: "#0d1117",
          }}>
            <UpgradeProModal
        ...
        {/* Hidden on the Pro slide — the modal provides its own CTAs */}
        {!isProSlide && (
          <div style={{
